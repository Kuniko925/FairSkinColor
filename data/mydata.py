...
def split_three_dataset(df):
    column = "labels"
    df_dummy, df_test = train_test_split(df, train_size=0.6, shuffle=True, random_state=42, stratify=df[column])
    df_train, df_valid = train_test_split(df_dummy, train_size=0.5, shuffle=True, random_state=42, stratify=df_dummy[column])
