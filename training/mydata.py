...
def uniformdataload(file_path, prediction=False):
    df = pd.read_csv(file_path)
    tmp = pd.DataFrame()
    tmp["labels"] = df["labels"].astype(str)
    tmp["skin tone"] = df["skin tone"].astype(str)
    tmp["predictions"] = df["predictions"].astype(str)
    #tmp["proba"] = df["proba"]
    tmp["filepath"] = df["filepath"]
    tmp["distance"] = df["distance"]
    return tmp

def uniformdataloadproba(file_path, prediction=False):
    df = pd.read_csv(file_path)
    tmp = pd.DataFrame()
    tmp["labels"] = df["labels"].astype(str)
    tmp["skin tone"] = df["skin tone"].astype(str)
